...
below, this workflow is foreign to most educators, but allows us to
scale and adapt more quickly and more cheaply than the centralized
...
\begin{quote}
\textbf{Why is this weird?}

Large-scale ad hoc collaboration is the norm in open source software
development, but Wikipedia aside, it is still rare in other fields. In
teaching, for example, people often use one another's slide decks as
starting points for their own courses, but rarely offer their changes
back to the original author in order to improve them. This is partly
because educators' preferred file formats (Word, PowerPoint, and PDF)
aren't handled gracefully by existing version control systems.  A
deeper cause, though, is that there isn't a culture of contribution,
particularly in higher education.

The question is, why not?  Many teachers are technically literate
enough to (for example) edit wiki pages; they have strong social and
professional ties, and are trying to solve the same problems.

\fixme{finish explaining}
\end{quote}
...
us dependent on university-quality Wi-Fi.)

The real problem with using virtual machines, though, is
\fixme{explain native install}.
...
It may seem like a trivial thing, but editing text is always harder
than we expect. We don't want to encourage people to use naive editors
like Notepad, and the two most popular legacy editors on Unix (Vi and
Emacs) are both usability nightmares. We now recommend a handful of
GUI editors, but it remains a stumbling block.
\end{quote}
